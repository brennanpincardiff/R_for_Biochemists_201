...
* Use dplyr to reformat data
* Use dplyr to summarise data
* Create bar plots, box & whisker plots and violin plots with ggplot2


The focus should be on a set of proteomics data that comes in a wide format but needs to be summarised, probed statistically and reformatted for plotting in various ways. 

Introduce box and whisker, violin plots, dots on plots and linking the dots for individual samples. 
...
### Tidy our data
The data is in a wide format `r View(data)`. Each observation is in a column. 
This is how we are used to seeing our data and we could graph it like this. 
However, things will work better if we turn the data into long format that is
more of the structure we like for Tidy Data. 
We do this using the gather() function. 
...
is easy. Our plotting function, ggplot() will summarise our data and plot the mean of our
data using geom_bar. 
...
We can differentiate the sizes of the numbers. It's not clear that it is much 
better than just the numbers but we do like to visualise the data. 

### Summarize as lines or show the individual data points
However, it has limits and there is a proposition that is hides more than it 
reveals and that is uses a lot of ink for showing the numbers. For example, 
it is not really better than just generating lines with now bars...
...
data_3 %>%
    group_by(key) %>%
    summarise(mean(value)) -> data_3_stats
colnames(data_3_stats) <- c("key", "value")
...
I have to confess that I don't really like this. What can we do better?
Well we can show the points to show the variation. 
